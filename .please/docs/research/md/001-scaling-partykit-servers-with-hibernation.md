...
url: "https://docs.partykit.io/guides/scaling-partykit-servers-with-hibernation/"
date: 2026-03-28
summary: "Covers the PartyKit Hibernation API, which offloads WebSocket connection memory to the platform, scaling a room from 100 to 32,000 connections. Explains how hibernation works, when to use it, its limitations, and patterns like partial state loading."
tags: [partykit, websockets, hibernation, scaling, realtime]
---
...
**Scale limit:** The limit to scale per room is typically 128MB memory. With Hibernation, the active WebSocket connections no longer count towards your party's 128MB memory limit, but the total memory footprint of your code, state, and dynamic allocations may still exceed that limit.
...
- If you're building with `Yjs` as `y-partykit` doesn't currently support Hibernation. (The PartyKit team is working on this.)
...
- The local `partykit dev` development environment does not hibernate. This means the behaviour of the code while developing can be different from behaviour on the hosted platform. (The PartyKit team is working on this.)
...
**PartyKit storage note:** The PartyKit `storage` API has its own built-in cache. Because of that, reading frequently the same key is very performant as the result is cached in memory instead of having to read it from disk every time.
...
export default class Server implements Party.Server {
  ...
  async onStart() {
    // BAD: we load full state from storage on start (may be big)
  ...
  async onMessage(websocketMessage: string) {
    ...
    if (event.type === "update") {
      ...
      this.items[event.id] = { ...item, ...event.data };
    };

    // BAD: we need to persist the whole state again (expensive)
